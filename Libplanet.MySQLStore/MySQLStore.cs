...
namespace Libplanet.MySQLStore
{
    ...
    public class MySQLStore : BaseStore
    {
        ...
        public override HashDigest<SHA256>? IndexBlockHash(Guid chainId, long index)
        {
            ...
            var idx = Select<ChainModel, byte[]>(
                ChainDbName,
                "Key",
                key);
        ...
        public override IEnumerable<TxId> IterateStagedTransactionIds()
        {
            ...
            using QueryFactory db = MySQLUtils.OpenMySQLDB(_connectionString, _compiler);
            Query query = db.Query(StagedTxDbName).Select(new[] { "Key", "Value" }).Where("Key", prefix);
            IEnumerable<StagedTransactionModel> stagedTransactions = query.Get<StagedTransactionModel>();

            foreach (var stagedTransaction in stagedTransactions)
            {
                byte[] key = stagedTransaction.Key;
        ...
        public override void Dispose()
        {
            ...
            db.Statement($"truncate table {ChainDbName}");
            db.Statement($"truncate table {BlockDbName}");
            db.Statement($"truncate table {TxDbName}");
            db.Statement($"truncate table {StagedTxDbName}");
            db.Statement($"truncate table {UpdatedAddressDbName}");
            db.Statement($"truncate table {CanonChainDbName}");
        }
